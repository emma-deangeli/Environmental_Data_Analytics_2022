...
> Add notes here: This all seems like it would be important for fish, plants, and other life to survive in lakes.
...
NTL.phys.data.PeterPaul2 <- filter(NTL.phys.data, lakename != "Central Long Lake" & 
                                     ...
                                     lakename != "Ward Lake" & lakename != "West Long Lake") # so the != just means without
...
NTL.phys.data.90to99 <- filter(NTL.phys.data, year4 >= 1990, year4 <= 1999)

```
Question: Why don't we filter using row numbers?

> Answer: The rows don't tell us anything useful about trends or patterns.
...
# 1. East Long, July 1998, 0.5
# 2. Hummingbird, July 2002, 0
# 3. Hummingbird, July 2002, 0
```
### Select
...
When we wrangle a raw dataset into a processed dataset, we create a code file that contains only the wrangling code. We then save the processed dataset as a new spreadsheet and then create a separate code file to analyze and visualize the dataset. Why do we keep the wrangling code separate from the analysis code?
 ...
 > Answer: We don't want to mess with the processed datasets so that we could do different analyses if need be.
