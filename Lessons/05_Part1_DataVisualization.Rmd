...
# How could you automatically assign a marker color to a variable? say color = variable
PMplot <- 
...
Nutrientplot3 <-
  ...
  geom_boxplot(aes(color = nutrient)) # Why didn't we use "fill"? we could have but not all the data are colorful when we do so... tbh just looks prettier with color
print(Nutrientplot3)
...
Nutrientplot5 <-
  ggplot(PeterPaul.chem.nutrients.gathered, aes(x = lakename, y = concentration)) +
  geom_violin(aes(color = nutrient)) #this isn't awesome because the violin doesn't show
print(Nutrientplot5)
