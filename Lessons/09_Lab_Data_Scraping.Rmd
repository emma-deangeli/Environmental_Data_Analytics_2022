...
# period gives us days of data, format = rdb gives us a csv file
...
```{r pull.and.plot.discharge.data}
#Set the URL
theURL1 <- 'https://waterdata.usgs.gov/nwis/uv?search_site_no=02085070&period=24&format=rdb'
# period gives us days of data, format = rdb gives us a csv file

#Get the data, which starts on line 30
gage_data1 <- read.table(theURL1,skip = 28,header=TRUE,sep='\t') 

#Update the column headers
colnames(gage_data1) = c("agency_cd", "site_no", "datetime", "tz_cd",
                        "discharge_cfs", "89192_00060_cd", "gage_ht_ft", "89193_00065_cd")

#Tidy the data
gage_data1 <- gage_data1 %>% 
  select(datetime, discharge_cfs,gage_ht_ft) %>% 
  mutate(datetime = ymd_hm(datetime))

#Plot
ggplot(gage_data1,aes(x=datetime,y=discharge_cfs)) + 
  geom_line() +
  labs(x = "Date",y = "Discharge (cfs)")
```
...
pcode = '00060' #discharge (in cubic feet per second)
...
```{r}
#Apply the API key
census_api_key("6ae0c99efda40bdeb18a93250de6d8f3b1401371", install = TRUE)
...
df_electric <- data.frame("States" = the_states,
                          "Retail_price" = as.numeric(the_price),
                          "Summer_capacity" = as.numeric(gsub(",","",the_capacity)),
                          "Net_generation" = as.numeric(gsub(",","",net_generation)),
...
scrape.it <- function(the_year){
  ...
  # Code from above (CFA): Fetch website
