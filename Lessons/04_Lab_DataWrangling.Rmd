...
class(NTL.phys.data$sampledate) #double checking that it worked
```
...
# dplyr filtering
NTL.phys.data.surface <- filter(NTL.phys.data, depth == 0) # only the data where the variable 'depth' is equal to 0
...
# using the year == language because it's an integer
...
> Answer: they don't give us any useful information about the dataset
...
#Exercise 4: Using the data from part 3, a pipe and the summarise() function find the mean surface temperature (hints: you will need to add another filter for depth==0). Make sure you eliminate NAs before computing the means
  # what do you mean by part 3? exercise 3? and should we do the mean surface temperatures for each lake?
...
NTL.irradiance.gathered <- pivot_longer(NTL.phys.data, irradianceWater:irradianceDeck, names_to = "irradiance_loc", values_to = "irradiance")


#Exercise 6: spread temperatureC into more than one column based on the depth

NTL.temperatureC.spread <- pivot_wider(NTL.phys.data, names_from = depth, values_from = temperature_C)
