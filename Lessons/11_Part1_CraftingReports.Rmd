...
* It's easy enough to do **bullet points** in R Markdowns
* It has *value* in creating clean reports about data
* Commenting without confusing it with your code
* Easy to share with other people
* Great for crafting reports
* Doing this in RStudio and version control with Git is seamless
...
Dataset names    | Relevant information
-----------------|----------------------
PeterPaul        | Nutrient loads for Peter and Paul Lake
Garinger Ozone   | The amount of ozone at the Garinger site
EPA.10 - EPA.19  | Datasets that include information on ozone and particulate matter
...
Nutrients_df <- read.csv("../Data/Raw/NTL-LTER_Lake_Nutrients_Raw.csv", stringsAsFactors = TRUE)
head(Nutrients_df)
Nutrients_df$sampledate <- as.Date(Nutrients_df$sampledate, format = "%m/%d/%y")
```
...
```{r}
Nutrients_revised_df <- Nutrients_df %>%
...
```{r include=FALSE}
Nitrogen_summaries <- 
  Nutrients_revised_df %>%
  group_by(lakename) %>%
  summarise(meanTN = mean(tn_ug), 
            minTN = min(tn_ug), 
            maxTN = max(tn_ug))

Phosphorus_summaries <- 
  Nutrients_revised_df %>%
    group_by(lakename) %>%
  summarise(meanTN = mean(tp_ug), 
            minTN = min(tp_ug), 
            maxTN = max(tp_ug))
```
...
```{r echo=FALSE}
knitr::kable(Nitrogen_summaries, caption = "Nitrogen Summaries")
knitr::kable(Phosphorus_summaries, caption = "Phosphorus Summaries")
...
```{r echo=FALSE}
ggplot(Nutrients_revised_df, aes(x = sampledate, y = tn_ug, color = lakename)) + 
    geom_point() + 
    scale_color_brewer(palette = "Dark2")
```

```{r echo=FALSE}
ggplot(Nutrients_revised_df, aes(x = sampledate, y = tp_ug, color = lakename)) +
    geom_point() + 
    scale_color_brewer(palette = "Dark2")
```
...
> There are some lakes in North Carolina that have more nutrient loads than the other. In particular, it looks like Hummingbird Lake, East Long Lake, and Central Long Lake all have higher nutrient loads than the others, both in terms of their average total phosphorus and average total nitrogen levels. We can see this within the visualization. It shows that while lake nutrient loads have varied over time, there are some lakes that are consistently lower on nutrient loads and some that are consistently higher.

----------------------------------------------------------------------------

> In analyzing this dataset, I might observe how each lake's nutrient loads have changed over time. I might also look at the correlation between nitrogen and phosphorus in these different lakes to see if they're connected. Further, I would see which lakes have statistically significant findings and eliminate those that do not from the analyses.
