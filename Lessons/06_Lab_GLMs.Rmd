...
> p-value < 0.05, reject the null hypotheses that miu >= 50. Ozone levels within standard of "good"
...
#Exercise 2:
O3.density <-
  ggplot(EPAair, aes(x = Ozone, fill = as.factor(Year))) + # "fill" goes within the aes function
  geom_density(alpha = 0.8) +
  scale_fill_manual(values = c("purple", "magenta")) +
  labs(x = "Ozone AQI vale", y = "Density", fill = "")
print(O3.density)
...
PM2.5.regression <- lm(data = EPAair, PM2.5 ~ Ozone)
summary(PM2.5.regression)

## p-value <2.2e-16, R-squared: 0.1316
...
EPAairaerosols <- 
  ...
  select(PM2.5, Ozone) %>%
  na.omit()
EPAairCorr <- cor(EPAairaerosols)
corrplot(EPAairCorr, method = "ellipse") # this turned out ugly and unhelpful
corrplot.mixed(EPAairCorr, method = "ellipse")
...
EPAAIC <- lm(data = EPAair.naomit, PM2.5 ~ Ozone + Site.Name + 
              COUNTY + Month + Date)

#Choose a model by AIC in a Stepwise Algorithm
step(EPAAIC)
EPAmodel <- lm(data = EPAair.naomit, PM2.5 ~ Ozone + Site.Name + 
              COUNTY + Month)
# step(EPAmodel) Awesome!! This line looks good because it has <none> as the first row meaning the AIC is the lowest value that it could be given the variables
summary(EPAmodel)

# it seems like the counties are absolutely not relevant, but for the site names, only some of them are statistically significant
